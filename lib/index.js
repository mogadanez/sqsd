...
class SQSProcessor {
    ...
    handleMessage (sqsMessage) {
        ...
        .then( async ([postResult]) => {
            debug(  "Received result from worker, MessageId: %s  statusCode:%s ", sqsMessage.MessageId, postResult.statusCode)
            if (!(postResult.statusCode < 200 || postResult.statusCode >= 300)) {
                ...
                error( "Worker respond  with status != 2XX, MessageId: %s  statusCode:%s ", sqsMessage.MessageId, postResult.statusCode )
        })
        .then(x=> {
    ...
    async tick  () {
        ...
        debug({count: data.Messages.length}, "Messages Received")
