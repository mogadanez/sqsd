...
SQSProcessor.prototype.postToWorker = function (messageBody, sqsMessage) {
    ...
    for(var name in sqsMessage.MessageAttributes) {
      var value = sqsMessage.MessageAttributes[name];
        ...
        .spread((postResult) => {
            ...
            if (!(postResult.statusCode < 200 || postResult.statusCode >= 300)) {
                return this.deleteMessage({
                    ...
                    .then((res)=> {
                        logger.info({MessageId: sqsMessage.MessageId, taskTime: new Date().getTime() - startTime}, "Message successful removed from sqs ")
                    })
            } else logger.error({
                MessageId: sqsMessage.MessageId,
                statusCode: postResult.statusCode
...
SQSProcessor.prototype.tick = function () {
    ...
    logger.debug("Start Polling For %s Messages", (this.options.maxMessages - this.processingMesages.length))
    this.polling = true;
